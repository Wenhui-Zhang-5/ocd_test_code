...
const nextWorkspaceSeqAndId = (state) => {
  ...
  const nextSeq = Math.max(state.workspaceCounter || 0, maxExistingSeq) + 1;
  state.workspaceCounter = nextSeq;
  return {
    seq: nextSeq,
    workspaceId: `WKS-${String(nextSeq).padStart(4, "0")}`
  };
};

export const recipeHubRecipeNameExists = (recipeName, excludeWorkspaceId = "") => {
  const target = String(recipeName || "").trim().toLowerCase();
  const exclude = String(excludeWorkspaceId || "").trim();
  if (!target) return false;
  const state = loadState();
  return (state.workspaces || []).some((workspace) => {
    if (!workspace || workspace.type !== "model") return false;
    if (exclude && String(workspace.id || "") === exclude) return false;
    const status = String(workspace.status || "").toLowerCase();
    if (status === "temp") return false;
    return String(workspace.recipeName || "").trim().toLowerCase() === target;
  });
};

export const createRecipeHubWorkspace = (payload = {}) => {
  const state = loadState();
  const recipeName = String(payload.recipeName || "").trim();
  if (!recipeName) {
    throw new Error("recipe_name is required");
  }
  if (recipeHubRecipeNameExists(recipeName)) {
    throw new Error(`Recipe name "${recipeName}" already exists.`);
  }
  const { seq, workspaceId } = nextWorkspaceSeqAndId(state);
  const workspace = {
    id: workspaceId,
    type: "model",
    status: "draft",
  ...
  saveRecipeSchema(workspace.id, {
    ...
    recipeName: workspace.recipeName,
...
export const cloneWorkspaceCaseCache = async ({
  sourceWorkspaceId,
  targetWorkspaceId,
  recipeName = ""
}) => {
  ...
  const sourceCache = await loadWorkspaceCaseCache(sourceId);
  if (!sourceCache || typeof sourceCache !== "object") return false;

  await clearWorkspaceCaseCache(targetId);

  const sectionEntries = Object.entries(sourceCache).filter(
    ([section, value]) => section && value && typeof value === "object"
  );
  const clonedSections = sectionEntries.map(([section, value]) => {
    const next = clone(value);
    if (section === "spectrum" && next?.selection) {
      next.selection.workspaceId = targetId;
    }
    if (section === "precision" && next?.selection) {
      next.selection.workspaceId = targetId;
    }
    if (section === "schema" && next?.recipeSchema) {
      next.recipeSchema.recipeName = String(recipeName || next.recipeSchema.recipeName || "").trim();
      next.recipeSchema.state = "draft";
    }
    return [section, next];
  });
  await Promise.all(
    clonedSections.map(([section, value]) => saveWorkspaceCaseCacheSection(targetId, section, value))
  );

  const sourceSchema =
    sourceCache?.schema?.recipeSchema ||
    loadRecipeSchema(sourceId) ||
    {};
  const nextSchema = {
    ...(sourceSchema || {}),
    recipeName: String(recipeName || sourceSchema?.recipeName || "").trim(),
    state: "draft"
  };
  if (targetId && nextSchema.modelID === sourceId) {
    nextSchema.modelID = "";
  }
  saveRecipeSchema(targetId, nextSchema);

  if (sourceCache?.spectrum?.selection) {
    setSpectrumSelection({
      ...clone(sourceCache.spectrum.selection),
      workspaceId: targetId
    });
  }
  if (sourceCache?.precision?.selection) {
    setPrecisionSelection({
      ...clone(sourceCache.precision.selection),
      workspaceId: targetId
    });
  }

  const sourceSpectrumRuntime = getSpectrumRuntimeCache(sourceId);
  if (sourceSpectrumRuntime) {
    setSpectrumRuntimeCache(targetId, sourceSpectrumRuntime);
  }
  const sourcePrecisionRuntime = getPrecisionRuntimeCache(sourceId);
  if (sourcePrecisionRuntime) {
    setPrecisionRuntimeCache(targetId, sourcePrecisionRuntime);
  }
...
export const createModelWorkspace = (payload) => {
  const state = loadState();
  const { seq: nextIndex, workspaceId } = nextWorkspaceSeqAndId(state);
...
export const fetchModelJson = async (modelID) => {
  const target = String(modelID || "").trim();
  const response = await fetch(`${MODELS_API_BASE}/get`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export const fetchModelExists = async (modelID) => {
  const target = String(modelID || "").trim();
  if (!target) {
    return { exists: false };
  }
  const response = await fetch(`${MODELS_API_BASE}/exists`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ model_id: target })
  });
  if (!response.ok) {
    throw new Error("Model exists check failed");
  }
  const payload = await response.json();
